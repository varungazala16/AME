...
def analyze_left_leg_stand(video_path, 
                           show_video=False, 
    ...
    """
    Analyzes a video to assess one-leg stand performance on the LEFT LEG ONLY.
    ...
    try:
        while cap.isOpened():
            ...
            currently_detected_left_stand = False
            ...
            if show_video:
                ...
                if left_ankle and right_ankle:
                    # Left Ankle (Supporting) - Magenta
                    cv2.circle(frame, (int(left_ankle.x * w), int(left_ankle.y * h)), 10, (255, 0, 255), -1)
                    # Right Ankle (Raised) - Cyan
                    cv2.circle(frame, (int(right_ankle.x * w), int(right_ankle.y * h)), 10, (255, 255, 0), -1)
                    # Draw a reference "ground" line based on the supporting ankle
                    cv2.line(frame, (0, int(left_ankle.y * h)), (w, int(left_ankle.y * h)), (0, 255, 255), 2)
                ...
                # Display Status Text
                status_text = "Standing on LEFT Leg" if is_standing_on_left_leg else "Both Feet Down"
                status_color = (0, 255, 0) if is_standing_on_left_leg else (0, 255, 255)
                ...
                cv2.imshow('Left Leg Stand Analysis', frame)
